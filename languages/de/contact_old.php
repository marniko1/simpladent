    ﻿    <div id="page" class="contacts">
        <div class="main_img" style="background-image: url('img/contact/simpl2.jpg')
        "></div>
        <div class="in">
            <div class="section main_section" id="contactsSection_1">
                <div class="main_block">
                    <h1>Kontakt</h1>
                    <div class="article">
                        <h2>Telefonische Anftagen</h2>
                        <p>+41 55 293 23 33</p>
                        <h2>Email:</h2>
                        <p><a href="mailto:info@simpladent.ua">info@simpladent.ch</a></p>
                    </div>
                </div>
                <div class="clear"></div>
            </div>

            <div class="section" id="contactsSection_2">
                <div class="article">
                    <h2>Unsere Standorte:</h2>
                    <p>Die Verwaltung der einzelnen Klinik-Partner ermöglicht die Platzierung von Patienten aus anderen Städten und Ländern.</p>
                </div>
                <div class="clear"></div>
                <div class="contactsMapBlock">
                    <script type="text/javascript">
                        var addressList = { 4: ['Simpladent GmbH <p></p><p>&nbsp;</p><p>+41 55 293 23 33</p><p>Aktuell werden nur Vermittlungen ins Ausland angenommen</p>' ]              }
                    </script>
                    <div id="mapBlock">
                        <div id="map"></div>
                    </div>
                            <div id="cities_list">
                            <!--  в аттрибут data-coords записываются координаты с google maps -->
                            <ul>
                                        <li data-id="4" data-coords="49.987460, 34.233098">Gommiswald</li>
                             </ul>
                        </div>
                                        <div class="clear"></div>
                    <div class="clearfix" id="addresses_list"></div>
                    <div class="clear"></div>
                </div>

            </div>
        </div>


    </div>